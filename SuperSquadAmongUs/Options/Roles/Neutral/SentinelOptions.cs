using MiraAPI.GameOptions;
using MiraAPI.GameOptions.Attributes;
using MiraAPI.GameOptions.OptionTypes;
using MiraAPI.Utilities;
using SuperSquadAmongUs.Roles.Neutral;
using TownOfUs.Modules.Localization;

namespace SuperSquadAmongUs.Options.Roles.Neutral;

public sealed class SentinelOptions : AbstractRoleOptionGroup<SentinelRole>
{
    public override string GroupName => TouLocale.Get("SuperSquadRoleSentinel", "Sentinel");

    [ModdedNumberOption("SuperSquadOptionSentinelKillCooldown", 5f, 120f, 2.5f, MiraNumberSuffixes.Seconds)]
    public float KillCooldown { get; set; } = 25f;

    [ModdedNumberOption("SuperSquadOptionSentinelExplodeCooldown", 5f, 120f, 2.5f, MiraNumberSuffixes.Seconds)]
    public float ExplodeCooldown { get; set; } = 60f;

    public ModdedNumberOption ExplosionRadius { get; set; } = new("SuperSquadOptionSentinelExplosionRadius", 0.25f, 0.05f, 1f, 0.05f,
        MiraNumberSuffixes.Multiplier, "0.00");

    [ModdedToggleOption("SuperSquadOptionSentinelImpostorVision")]
    public bool ImpostorVision { get; set; } = true;

    [ModdedToggleOption("SuperSquadOptionSentinelCanVent")]
    public bool CanVent { get; set; }
}