...
/// and is a no-op unless the local player has an active aim window. The single role-agnostic
/// <see cref="GrantedSnipeButton"/> covers every role that can be *granted* Snipe (Gooper, Kirby, and
/// any future one) - one line, no per-role button to add when the roster grows.
...
public static class SniperAimPatch
{
    ...
    public static void Postfix()
    {
        ...
        try
        {
            CustomButtonSingleton<GrantedSnipeButton>.Instance.HandleAimFrame();
        }
        catch (Exception e)
        {
            Error($"Granted Snipe: HandleAimFrame threw: {e}");
        }
    }
}
