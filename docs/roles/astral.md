...
- **Possible interaction gap with Kirby's swallow, unconfirmed in-game.** `AstralFormModifier.OnDeactivate` has no check for being simultaneously carried (e.g. swallowed by Kirby) — if reachable, the phase's own failed-return timer could fire independently of the carry state, snapping the swallowed player's position and forcing a self-kill/linger transition while Kirby still has them pinned. Whether a phased Astral (disabled collider) is even swallow-targetable is unresolved from code alone — needs an in-game test with Kirby swallowing a phased Astral, not more code review. See `docs/roles/kirby.md`.
