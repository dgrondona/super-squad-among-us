...
**Meeting digestion.** `PelicanEvents.StartMeetingEventHandler` runs before vote areas are built: every devoured player dies via `Exiled()` (exile-style, no body). Runs deterministically on every client since devoured state arrived via synced modifiers. The kill loop itself lives in `PelicanEvents.DigestStomach()`, a shared helper also called from `PelicanRole.WinConditionMet()` (see below) so a win hit mid-round digests the stomach too — this makes `WinConditionMet()` an exception to the "pure predicate" pattern every other role's win check follows; it's safe only because the digest is idempotent (already-dead targets are skipped via `HasDied()`) and it's called on every client that evaluates the win check, not just host.
